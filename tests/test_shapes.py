...
class TestCase(EphemeralProject):
    ...
    def test_setup_fixtures(self):
        ...
        copyfile(
            f"{self.project}/tests/fixtures/shapes.py", f"{self.repo.name}/shapes.py"
        )
        copyfile(
    ...
    def test_write(self):
        result = []
        result.append(
            self.runner.invoke(sntl, ["arg", "write", "required_arg", "given_value"])
        )
        result.append(
            self.runner.invoke(sntl, ["env", "write", "required_env", "given_value"])
        )
        assert all(result)
