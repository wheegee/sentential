...
class TestContext(object):
    ...
    def test_repository_name_malformed_dockerfile(
        self, ontology: Ontology, repo, project, invoke
    ):
        copyfile(
            f"{project}/tests/fixtures/files/Dockerfile.malformed",
            f"{repo.name}/Dockerfile",
        )
        with pytest.raises(ContextError):
            ontology.context.repository_name
        remove("./Dockerfile")
    ...
    def test_repository_url(self, ontology: Ontology):
        assert (
            ...
            == "123456789012.dkr.ecr.us-west-2.amazonaws.com/testing"
        )
