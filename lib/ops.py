...
class Ops:
    ...
    def build(self, tag: str = "latest"):
        spec = Spec(prefix=self.config.repository_name,
        ...
        clients.docker.build(
                ...
                tags=[
                    f"{self.config.repository_name}:{tag}"
                ]
            )
    ...
    def emulate(self, tag: str = "latest"):
        ...
        image = clients.docker.image.inspect(f"{event.detail.repository_name}:{event.detail.image_tag}")
