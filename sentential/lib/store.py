...
class Store:
    ...
    def read(self):
        ...
        data = [ [ p['Name'], p['Value'], p['Version'], p['LastModifiedDate'], p['Type'] ] for p in parameters if self.type == p['Type'] ]
        print(tabulate(data, headers=["Key", "Value", "Version", "LastModified", "Type"], tablefmt='tsv'))

    def parameters(self):
        data = { p['Name'].replace(f"/{self.repository_name}/",""): p['Value'] for p in self.fetch() if self.type == p['Type'] }
        return SimpleNamespace(**data)

    def delete(self, key: str):
        return clients.ssm.delete_parameter(
            Name=f"/{self.repository_name}/{key}"
        )
